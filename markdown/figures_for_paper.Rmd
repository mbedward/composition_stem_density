...
get_indices <- function(ptn, n.expected = NULL) {
  ii <- str_detect(colnames(samples), ptn)
...
for (k in 1:nrow(samples)) {
  ...
  lv.coefs <- matrix(samples[k, ii.lvcoefs], nrow = nspp)

  intercept_var[k, ] <- lv.coefs[,1]^2
  lv_var[k, ] <- rowSums(lv.coefs[, 2:(1+nlv)]^2)
  
  # contribution of stem density
  stem.coefs <- matrix(samples[k, ii.stemcoefs], nrow = nspp)
  mu.stems <- matrix(samples[k, ii.mu.stems], nrow = nplots)
  ...
  flood.coefs <- matrix(samples[k, ii.floodcoefs], nrow = nspp)
  ...
  raneff_var[k, ] <- samples[k, ii.site.sigma]^2 + samples[k, ii.siteplot.sigma]^2
}
