...
Xstems.dense.In <- cbind(qdat[1, 1:2, drop=FALSE],
                         qdat[9, 3:4, drop=FALSE], 
                         qdat[1, 5:6, drop=FALSE])

# small 10%, intermediate 10%, large 90%
Xstems.dense.Lg <- cbind(qdat[1, 1:4, drop=FALSE], 
                         qdat[9, 5:6, drop=FALSE])

# small 90%, intermediate 90%, large 10%
Xstems.dense.SmIn <- cbind(qdat[9, 1:4, drop=FALSE], 
                           qdat[1, 5:6, drop=FALSE])

# small 90%, intermediate 10%, large 90%
Xstems.dense.SmLg <- cbind(qdat[9, 1:2, drop=FALSE],
                           qdat[1, 3:4, drop=FALSE], 
                           qdat[9, 5:6, drop=FALSE])
...
Function to create graph data given predicted occurrence probabilities for a subset of species and hypothetical stands:
...
fn_gdata <- function(dat.prob, 
  ...
  # Classify species on probability of occurrence across hypothetical
  # stands where all size groups are varied ('all10' - 'all90')
  #
  #ii <- str_detect(Xstems.lookup$label, "^all")
  #ipred <- Xstems.lookup$prediction.index[ii]
  #
  #d <- dist(t(dat.prob[ipred, ]))
  #h <- hclust(d, method = "centroid")
  #specieslevels <- h$labels[h$order]
  
  
  # Alternatively, order species on their mean probability of 
  # occurrence when all tree sizes are at the 90th percentile 
  # of modelled stem density
  ii <- Xstems.lookup$label == "all90"
  ipred <- Xstems.lookup$prediction.index[ii] 
  p <- dat.prob[ipred, ]
  specieslevels <- colnames(dat)[order(p)]
  ...
  dat.gg <- as.data.frame(dat.prob) %>%
    ...
    mutate(prob = round(prob, 2),
           species = factor(species, levels = specieslevels)) %>%
...
gg_pdf(gg, 
       "model_chosen_hypothetical_general_native_spp.pdf", 
...
gg_pdf(gg, 
       "model_chosen_hypothetical_general_exotic_spp.pdf", 
       pagesize("A3", "portrait"))
...
gg_pdf(gg, "model_chosen_hypothetical_extreme_native_spp.pdf", 
       pagesize("A3", "portrait"))
...
gg_pdf(gg, "model_chosen_hypothetical_extreme_exotic_spp.pdf", 
       pagesize("A3", "portrait"))
