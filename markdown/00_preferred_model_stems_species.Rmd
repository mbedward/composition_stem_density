...
# species as columns
dat <- apply(res, MARGIN = c(1,2), function(lp) pnorm(mean(lp)))

colnames(dat) <- SpeciesSubset$workingname

# Classifiy species on probability of occurrence across all
# hypothetical stands: general and dense
#
d <- dist(t(dat))
...
dat.gg <- as.data.frame(dat) %>%
  ...
  # Subset of 'general' stands
  filter(!str_detect(label, "dense")) %>%
  
  # To simplify the graph, just keep the data for stands 
  # at 20, 40, 60, 80th percentiles
  #filter(str_detect(label, "[2468]0$")) %>%
  
  # Vars for graph ordering
...
gg <- ggplot(data = dat.gg) +
  ...
  scale_fill_gradient(low = "#ffffff", high = "#000000", trans = trans_species_prob) +
...
gg_pdf(gg, "model_chosen_hypothetical_general_spp.pdf", pagesize("A3", "portrait"))
...
dat.gg <- as.data.frame(dat) %>%
  ...
  # Subset to data for extreme stands
  filter(str_detect(label, "dense")) %>%
  
  mutate(label = str_replace(label, "dense ", ""),
         label = factor(label, 
                        levels = c("Sm", "In", "Lg", "Sm.In", "Sm.Lg", "In.Lg"),
...
gg_pdf(gg, "model_chosen_hypothetical_extreme_spp.pdf", pagesize("A3", "portrait"))
...
Versions of the above graphs with species ordered by similarity in response to stem density.
